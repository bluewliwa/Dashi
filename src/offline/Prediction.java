...
public class Prediction {
	...
	public static void main(String[] args) {
		...
		/*
		 * var map = function() { if (this.item == "0634029363" && this.rating
		 * == 2) { emit(this.user, 1); } if (this.item == "B000TZTPQ6" &&
		 * this.rating == 5) { emit(this.user, 1); } if (this.item ==
		 * "B001QE994M" && this.rating == 4) { emit(this.user, 1); } if
		 * (this.item == ¡°B001QE997E" && this.rating == 5) { emit(this.user, 1);
		 * }}
		 */
